...
function LinksExample() {
    ...
    const fetchMovies = async () => {
      ...
      let url = "";
      const urlPage = `${page}`;
      url = `${configData.SERVER_URL}articles?_embed&status=publish&per_page=${urlPage}`;
      try {
    ...
    const fetchNos = async () => {
      //setLoading(true);
      let cat = "";
      cat = `${configData.SERVER_URL}categories/234 `;
      try {
...
<h2 className="txt-main">Articles</h2>
...
<section className="section text-center mb-3">
        {loading && <h2 className="loading">Loading Articles...</h2>}
